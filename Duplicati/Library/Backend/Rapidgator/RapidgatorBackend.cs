...
public class RapidgatorBackend : IStreamingBackend, IBackend, IDynamicModule, IDisposable
{
    ...
    public async Task PutAsync(string remotename, Stream stream, CancellationToken cancelToken)
    {
        ...
        string uploadId = uploadRequestResponse.Response.Upload.UploadId;
        ...
        await RetryHelper.Retry(async () =>
        {
            ...
            {
                content.Add(new StreamContent(stream), "file", remotename);
        ...
        while (uploadInfo.State == 1)
        {
            ...
            url = $"{API_URL}file/upload_info?upload_id={uploadId}";
            UploadResponse uploadInfoResponse = await _clientHelper.GetJsonDataAsync<UploadResponse>(url, cancelToken);
