...
namespace Duplicati.Library.Main.ForestHash.Database
{
    public partial class LocalRestoredatabase
    {
        ...
        private class FileToRestoreEnumerable : IEnumerable<IFileToRestore>
        {
            private class FileToRestoreEnumerator : IEnumerator<IFileToRestore>
            {
                private class FileToRestore : IFileToRestore
                {
                    private System.Data.IDataReader m_reader;
                    public FileToRestore(System.Data.IDataReader reader)
                    {
                        m_reader = reader;
                    }
                ...
                private System.Data.IDbCommand m_command;
                private string m_tablename;
                private FileToRestore m_file;

                public FileToRestoreEnumerator(System.Data.IDbConnection connection, string tablename)
                {
                    m_connection = connection;
                    m_tablename = tablename;
                    this.Reset();
                ...
                {
                    m_command = m_connection.CreateCommand();
                    m_command.CommandText = string.Format(@"SELECT ""{0}"".""TargetPath"", ""Blockset"".""FullHash"" FROM ""{0}"",""Blockset"" WHERE ""{0}"".""BlocksetID"" = ""Blockset"".""ID"" ", m_tablename);
                    m_reader = m_command.ExecuteReader();
                    m_file = new FileToRestore(m_reader);
                }
            }
            ...
            public FileToRestoreEnumerable(System.Data.IDbConnection connection, string tablename)
            {
                m_connection = connection;
                m_tablename = tablename;
            }

            public IEnumerator<IFileToRestore> GetEnumerator()
            {
                return new FileToRestoreEnumerator(m_connection, m_tablename);
            }
