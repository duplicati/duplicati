                ...
                var sink = CoCoL.AutomationExtensions.RunTask(
                    ...
                    async self =>
                    {
                        while (true)
                        {
                            ...
                            {
                                Logging.Log.WriteVerboseMessage(LOGTAG, "FailedAttributeRead",
                            ...
                            if (isSymlink)
                                ...
                                {
                                    Logging.Log.WriteExplicitMessage(LOGTAG, "SymlinkTargetReadFailure", ex,
                                        "Failed to read symlink target for path: {0}", path);
                                }

                            if (isSymlink && m_options.SymlinkPolicy == Options.SymlinkStrategy.Store &&
                            ...
                            if (!((fa & FileAttributes.Directory) == FileAttributes.Directory))
                            {
                                ...
                                {
                                    Logging.Log.WriteVerboseMessage(LOGTAG, "SizeReadFailed",
                                        "Failed to read length of file {0}: {1}", path, ex.Message);
                                }


                                if (m_options.SkipFilesLargerThan == long.MaxValue ||
                                    m_options.SkipFilesLargerThan == 0 || size < m_options.SkipFilesLargerThan)
                                    Logging.Log.WriteVerboseMessage(LOGTAG, "IncludeFile",
                                        "Including file: {0} ({1})", path,
                                        size < 0
                                            ? "unknown"
                                            : Duplicati.Library.Utility.Utility.FormatSizeString(size));
                                else
                                    Logging.Log.WriteVerboseMessage(LOGTAG, "ExcludeLargeFile",
                                        "Excluding file due to size: {0} ({1})", path,
                                        size < 0
                                            ? "unknown"
                                            : Duplicati.Library.Utility.Utility.FormatSizeString(size));
                            }
