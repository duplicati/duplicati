        ...
        public static Task Run(Snapshots.ISnapshotService snapshot, Options options, BackupStatsCollector stats, BackupDatabase database)
        {
            ...
            async self =>
            {
                ...
                while (true)
                {
                    ...
                    e.MetaHashAndSize = options.StoreMetadata ? Utility.WrapMetadata(MetadataGenerator.GenerateMetadata(e.Path, e.Attributes, options, snapshot), options) : EMPTY_METADATA;
