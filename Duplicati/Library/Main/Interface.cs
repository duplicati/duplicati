...
namespace Duplicati.Library.Main
{
    ...
    public class Interface : IDisposable, LiveControl.ILiveControl
    {
        ...
        public string Backupv2(string[] sources)
        {
        ...
        public string Restorev2(string[] target)
        {
        ...
        public string Backup(string[] sources)
        {
            ...
            using (new Logging.Timer("Backup from " + string.Join(";", sources) + " to " + m_backend))
            {
                ...
                {                    
                    ...
                    foreach(string s in DynamicLoader.CompressionLoader.Keys)
                        if (string.Equals(s, m_options.CompressionModule, StringComparison.InvariantCultureIgnoreCase))
                        {
                    ...
                    if (!compressionFound)
                        throw new Exception(string.Format(Strings.Interface.CompressionModuleNotFoundError, m_options.CompressionModule, string.Join(", ", DynamicLoader.CompressionLoader.Keys)));

                    if (!m_options.NoEncryption)
                    {
                        bool encryptionFound = false;
                        foreach (string s in DynamicLoader.EncryptionLoader.Keys)
                            if (string.Equals(s, m_options.EncryptionModule, StringComparison.InvariantCultureIgnoreCase))
                            {
                        ...
                        if (!encryptionFound)
                            throw new Exception(string.Format(Strings.Interface.EncryptionModuleNotFoundError, m_options.EncryptionModule, string.Join(", ", DynamicLoader.EncryptionLoader.Keys)));
                    }
                    ...
                    {
                        List<KeyValuePair<ManifestEntry, CompressionWrapper>> patches = new List<KeyValuePair<ManifestEntry, CompressionWrapper>>();
                        if (!full)
                        {
                        ...
                        using (RSync.RSyncDir dir = new Duplicati.Library.Main.RSync.RSyncDir(manifest.SourceDirs, bs, backend.Metadata, m_options.Filter, m_options.CompressionHints, patches))
                        {
                            ...
                            while (!done && totalsize < m_options.MaxSize)
                            {
                                ...
                                using (Utility.TempFile contentfile = new Duplicati.Library.Utility.TempFile(System.IO.Path.Combine(tempVolumeFolder, Guid.NewGuid().ToString())))
                                {
                                    ...
                                    using (CompressionWrapper signaturearchive = CompressionWrapper.GetModule(m_options.CompressionModule, signaturefile, m_options.RawOptions))
                                    using (CompressionWrapper contentarchive = CompressionWrapper.GetModule(m_options.CompressionModule, contentfile, m_options.RawOptions))
                                    {
                ...
                {
                    //May be overwritten if this is a partial success
                    m_result = ex;
        ...
        public string RestoreControlFilesv2(string target)
        {
        ...
        {
            if (!string.IsNullOrEmpty(new ForestHash.FhOptions(m_options.RawOptions).Fhdbpath))
                return RestoreControlFilesv2(target);
                        ...
                        foreach (ManifestEntry be in flatlist)
                        {
                            if (be.Volumes.Count > 0)
                                using(Utility.TempFile z = new Duplicati.Library.Utility.TempFile())
                                {
                                    ...
                                    using (CompressionWrapper fz = CompressionWrapper.GetModule(be.Volumes[0].Key.Compression, z, m_options.RawOptions))
                                    {
                                        bool any = false;
        ...
        public string DeleteAllButN ()
        {
            ...
            try
            {
                using (backend = new BackendWrapper(stats, m_backend, m_options))
                {
                    if (OperationStarted != null)
        ...
        {
            if (!string.IsNullOrEmpty(new ForestHash.FhOptions(m_options.RawOptions).Fhdbpath))
                throw new Exception("DeleteAllButNFull not supported with ForestHash");
        ...
        public string DeleteOlderThanv2()
        {
            var rs = new RestoreStatistics(DuplicatiOperationMode.DeleteOlderThan);
            SetupCommonOptions(rs);
        ...
        public string Cleanupv2()
        {
            ...
            using (var handler = new ForestHash.Operation.CleanupHandler(m_backend, fhopts, rs))
                handler.Run();
        ...
        public List<string> ListCurrentFilesv2()
        {
        ...
        private List<KeyValuePair<ManifestEntry, CompressionWrapper>> FindPatches(BackendWrapper backend, List<ManifestEntry> entries, string tempfolder, bool allowHashFail, CommunicationStatistics stat)
        {
            List<KeyValuePair<ManifestEntry, CompressionWrapper>> patches = new List<KeyValuePair<ManifestEntry, CompressionWrapper>>();
            ...
            using (new Logging.Timer("Reading incremental data"))
            {
                ...
                foreach (ManifestEntry be in entries)
                {
                    ...
                    foreach (KeyValuePair<SignatureEntry, ContentEntry> bes in be.Volumes)
                    {
                        ...
                        patches.Add(new KeyValuePair<ManifestEntry, CompressionWrapper>(be, CompressionWrapper.GetModule(bes.Key.Compression, filename, m_options.RawOptions)));
                    }
                }
            }
        ...
        public List<ManifestEntry> GetBackupSets()
        {
            ...
            if (!string.IsNullOrEmpty(new ForestHash.FhOptions(m_options.RawOptions).Fhdbpath))
                throw new Exception("GetBackupSets not supported with ForestHash");

            try
            {
                using (BackendWrapper backend = new BackendWrapper(stats, m_backend, m_options))
                {
                    List<ManifestEntry> res = backend.GetBackupSets();
                    m_result = res;
                    return res;
                }
            }
        ...
        {
            if (!string.IsNullOrEmpty(new ForestHash.FhOptions(m_options.RawOptions).Fhdbpath))
                throw new Exception("ListActualSignatureFiles not supported with ForestHash"); ;
            ...
            try
            {
                using (BackendWrapper backend = new BackendWrapper(stats, m_backend, m_options))
                {
                    ...
                    using (Utility.TempFolder folder = new Duplicati.Library.Utility.TempFolder())
                    {
                        ...
                        foreach (KeyValuePair<ManifestEntry, CompressionWrapper> entry in FindPatches(backend, new List<ManifestEntry>(new ManifestEntry[] { bestFit }), folder, false, stats))
                        ...
                        {
                            List<KeyValuePair<RSync.RSyncDir.PatchFileType, string>> res = dir.ListPatchFiles(patches);
                            m_result = res;
                            return res;
                        }
                    }
        ...
        {
            if (!string.IsNullOrEmpty(new ForestHash.FhOptions(m_options.RawOptions).Fhdbpath))
                throw new Exception("VerifyBackupChain not supported with ForestHash");
        ...
        public List<KeyValuePair<string, DateTime>> FindLastFileVersionv2()
        {
                    ...
                    foreach (ManifestEntry mf in workList)
                    {
                        ...
                        foreach(KeyValuePair<SignatureEntry, ContentEntry> e in mf.Volumes)
                            using (Utility.TempFile tf = new Duplicati.Library.Utility.TempFile())
                            {
                                ...
                                RSync.RSyncDir.ContainsFile(mfi, filesToFind, CompressionWrapper.GetModule(e.Key.Compression, tf, m_options.RawOptions));
