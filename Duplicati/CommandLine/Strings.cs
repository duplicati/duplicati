...
{2}", actualcommands, expectedcommands, string.Join(System.Environment.NewLine, commands ?? new string[0])); }
        ...
        public static string DefaultFiltersLong(string platform, string common, string windows, string osx, string linux, string all) { return LC.L(@"Exclude files that match the given filter sets. Which default filter sets should be used. Valid sets are ""{0}"", ""{1}"", ""{2}"", ""{3}"", ""{4}"", and ""{5}"". ""{0}"" selects the filters that apply for the current operating system, ""{1}"" selects the filters that apply to all operating systems, and ""{5}"" selects all filters. If this parameter is set with no value, the ""{0}"" set will be used.", platform, common, windows, osx, linux, all); }
