  ...
  check(b: AddOrUpdateBackupData, opt: BackupOptions): Promise<void> {
    if (opt.encryptionModule.length > 0 && !this.passphrase.checkGpgAsymmetric(opt.encryptionModule, opt.extendedOptions)) {
      if (opt.passphrase.length == 0) {
        this.dialog.dialog('Missing passphrase',
          'You must enter a passphrase or disable encryption');
        return Promise.reject(new RejectToStep(0));
      }
      if (opt.passphrase !== opt.repeatPassphrase) {
        this.dialog.dialog('Non-matching passphrase',
          'Passphrases are not matching');
        return Promise.reject(new RejectToStep(0));
      }
    }
  ...
  check(b: AddOrUpdateBackupData, opt: BackupOptions): Promise<void> {
    ...
    if (opt.hasGeneratedPassphrase && opt.encryptionModule.length > 0) {
      return new Promise<void>((resolve, reject) => {
        this.dialog.dialog('Autogenerated passphrase',
          'You have generated a strong passphrase. Make sure you have made a safe copy of the passphrase, as the data cannot be recovered if you lose the passphrase.',
          ['Cancel', 'Yes, I have stored the passphrase safely'],
          (ix) => {
            if (ix === 0) {
...
export class CheckForChangedPassphrase implements BackupChecker {
  ...
  check(b: AddOrUpdateBackupData, opt: BackupOptions): Promise<void> {
    ...
    if (encryptionEnabled && prevEncryptionEnabled && prevPassphrase !== opt.passphrase) {
      ...
      return new Promise((resolve, reject) => {
        this.dialog.dialog('Encryption changed',
          'You have changed the encryption mode. This may break stuff. You are encouraged to create a new backup instead',
          ['Cancel', 'Yes, I\'m brave!'],
          (ix) => {
            if (ix == 1) {
              resolve();
            } else {
...
export class CheckForDisabledEncryption implements BackupChecker {
  ...
  check(b: AddOrUpdateBackupData, opt: BackupOptions): Promise<void> {
    if (opt.encryptionModule != '' || b.Backup.TargetURL.startsWith('file://') || !this.hasAnyEncryptionModules) {
      ...
      return new Promise((resolve, reject) => {
        this.dialog.dialog('No encryption',
          'You have chosen not to encrypt the backup. Encryption is recommended for all data stored on a remote server.',
          ['Cancel', 'Continue without encryption'],
          (ix) => {
            if (ix === 0) {
              reject(new RejectToStep(0));
            } else {
...
export class WarnWeakPassphrase implements BackupChecker {
  ...
  check(b: AddOrUpdateBackupData, opt: BackupOptions): Promise<void> {
    ...
    if (opt.encryptionModule != '') {
      ...
      if (strength <= 2) {
        return new Promise((resolve, reject) => {
          this.dialog.dialog('Weak passphrase',
            'Your passphrase is easy to guess. Consider changing passphrase.',
            ['Cancel', 'Use weak passphrase'],
            (ix) => {
              if (ix == 0) {
                reject(new RejectToStep(0));
              } else {
