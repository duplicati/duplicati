...
namespace Duplicati.Server.WebServer
{
    public class Server
    {
        ...
        /// </summary>
        private Nancy.Hosting.Self.NancyHost m_server;
        ...
        public const string API_V1_PREFIX = "/api/v1";
        ...
        public Server(IDictionary<string, string> options)
        {
            ...
            var booter = new BootStrapHandler(options);
            ...
            foreach(var p in ports)
                try
                {
                    ...
                    var host = new NancyHost(new Uri("http://" + listenInterface.ToString() + ":" + p), booter);
                    host.Start();
        ...
        private class BootStrapHandler : DefaultNancyBootstrapper
        {
            private readonly string m_webroot;
            private readonly string m_customized;

            public BootStrapHandler(Dictionary<string, string> options)
            {
                ...
                if (install_webroot != webroot && System.IO.Directory.Exists(System.IO.Path.Combine(install_webroot, "customized")))
                    m_customized = System.IO.Path.Combine(install_webroot, "customized");
                else
                    m_customized = null;
            ...
            protected override void ConfigureConventions(NancyConventions nancyConventions)
            {
                ...
                nancyConventions.StaticContentsConventions.Add(StaticContentConventionBuilder.AddDirectory("/", m_webroot));
                if (!string.IsNullOrWhiteSpace(m_customized))
                    nancyConventions.StaticContentsConventions.Add(StaticContentConventionBuilder.AddDirectory("/customized/", m_customized));
            }
        }
