...
namespace AutoUpdateBuilder
{
    public class Program
    {
        public static int Main(string[] _args)
        {
            ...
            if (!string.IsNullOrWhiteSpace(keyfile) && string.IsNullOrWhiteSpace(keyfilepassword))
            {
                Console.WriteLine("Enter keyfile passphrase: ");
                keyfilepassword = Console.ReadLine().Trim();
            }

            if (!string.IsNullOrWhiteSpace(keyfile) && !System.IO.File.Exists(keyfile))
            {
                Console.WriteLine("Keyfile not found, creating new");
                var newkey = System.Security.Cryptography.RSA.Create().ToXmlString(true);
            ...
            var privkey = (System.Security.Cryptography.RSACryptoServiceProvider)System.Security.Cryptography.RSA.Create();
